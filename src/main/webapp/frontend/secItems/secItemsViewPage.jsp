      ...
      <nav class="nav">
        ...
        <ul class="nav_list">
	        <c:forEach var="secCategoryVO" items="${secCategorylist}">
	         	 <li>
		         	 	<a href="<%=request.getContextPath() %>/sec_items/SecItemsServlet.do?action=get_BY_CATE&shCateID=${secCategoryVO.shCateID}">
    ...
    <main class="main">
      ...
      <c:forEach var="secPicsVO" items="${list}">
        <li>
          <a href="#">
            <div class="img_block">
              <img src="<%= request.getContextPath()%>/sec_pics/SecPicsReader?sh_id=${secPicsVO.shID}">
            </div>
            <span class="item_text">這是商品名稱這是商品名稱這是商品名稱這是商品名稱</span>
            <span class="item_price">這是價格</span>
            <a href="#" class="btn btn-buy">放入購物車</a>
          </a>
        </li>
      </c:forEach>
